"""ESMValTool CMORizer for Scripps data.
...
Download and processing instructions
    Download the following file:
        monthly_[flask/in_situ]_co2_[station].csv
...
LON_COORD = iris.coords.DimCoord([204.4], var_name='lon',
...
def _extract_variable(short_name, var, cfg, filepath, out_dir):
    ...
    attrs['mip'] = var['mip']
    attrs['version'] = re.split('[_,.]', filepath)[-2].upper()
...
def cmorization(in_dir, out_dir, cfg, _):
    ...
    filepath_pattern = os.path.join(in_dir, cfg['filename'])

    # Run the cmorization
    for (short_name, var) in cfg['variables'].items():
        logger.info("CMORizing variable '%s'", short_name)
        for station in var['stations']:
            filepath = filepath_pattern.replace("*", var['stations'][station],
                                                1)
            filepath = filepath.replace("*", station.lower(), 1)

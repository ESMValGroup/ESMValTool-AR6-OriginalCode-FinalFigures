...
#                R. Kazeroni (DLR) (resolution grouping for TM90)
# #############################################################################
# Description
...
#   20201113-kazeroni_remi: modified to average over resolution levels (LR, HR)
...
for (model_idx in c(1:(length(models_dataset)))) {
  ...
  if (is.element(dataset, modelgroup_LR)) {
    models_projects[model_idx] <- "LR" # gather all LR datasets
  }
  if (is.element(dataset, modelgroup_HR)) {
    models_projects[model_idx] <- "HR" # gather all HR datasets
  }
  for (seas in seasons) {
...
##
  ...
  for (season in seasons) {
    ...
    for (project in project_list){
      ...
      if (length(field_exp_mean) > 0) { # plot mean by resolution level or obs data if available
        if (project != "OBS") { # mean computed only for LR and HR datasets
          ...
          text_legend <- c(text_legend, paste(reso_legend[i_reso_legend], ":", n_datasets, "datasets"))
          i_reso_legend <- i_reso_legend + 1

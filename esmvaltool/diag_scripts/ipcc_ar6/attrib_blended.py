...
#  print ('attrib_warming inputs:',beta,betaCI,dec_warming,ci95_dec_warming,ci95_beta_obs)
...
#        if mm > 3: continue
        ...
        for exp in grouped_model_input_data:
            ...
            for ee, ensemble in enumerate(grouped_exp_input_data):
                ...
                #Hack to calculate uncertainty in obs warming.
...
#        exit('Obs not recognised')
    ...
    for mm, dataset in enumerate(grouped_input_data):
        ...
        if mm==10:
          ...
          col='C'+str(mm)
        plt.plot(years,mean_diag[:,0,mm],color=col,linewidth=4,label=dataset+'_ALL')
        plt.plot(years,mean_diag[:,1,mm],color=col,linewidth=4,ls='--')

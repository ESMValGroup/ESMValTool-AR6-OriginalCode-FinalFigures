...
#   Define variables for D&A analysis
    ...
    for mm, dataset in enumerate(grouped_input_data):
        ...
        att_out[dataset]=da.tls(xr,yr,ne=ens_sizes[[0,1],mm],cn1=cn1,cn2=cn2,flag_2S=1)
        print (att_out[dataset]['betaCI'][0,:]*(mean_dec_warming[0,mm]-mean_dec_warming[1,mm]))
        ...
        plt.plot([mm+0.9,mm+0.9],numpy.transpose(att_out[dataset]['betaCI'][0,:]*(mean_dec_warming[0,mm]-mean_dec_warming[1,mm])),color='red',linewidth=4,label=label1)
        plt.plot([mm+1.1,mm+1.1],numpy.transpose(att_out[dataset]['betaCI'][1,:]*mean_dec_warming[1,mm]),color='blue',linewidth=4,label=label2)
    ...
    for mm, dataset in enumerate(grouped_input_data):
        ...
        plt.plot([mm+1.1,mm+1.1],numpy.transpose(att_out[dataset]['betaCI'][1,:]),color='blue',linewidth=4,label=label2)
    plt.plot([0,3],[0,0],color='black',linewidth=1,ls='--')
    ...
    for mm, dataset in enumerate(grouped_input_data):
        ...
        plt.plot(years,mean_diag[:,1,mm],color='blue',linewidth=4,label=dataset+'_NAT',ls=myls)

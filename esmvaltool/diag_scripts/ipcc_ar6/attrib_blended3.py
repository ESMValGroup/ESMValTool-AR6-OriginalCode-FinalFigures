...
#   Define variables for D&A analysis
    ...
    att_out={}
    for mm, dataset in enumerate(grouped_input_data):
        (xr,yr,cn1,cn2)=da.reduce_dim(mean_diag[:,[0,1,3],mm],had4_diag[:,None],anom[:,0:int(anom_index/2)],anom[:,int(anom_index/2):anom_index])

        att_out[dataset]=da.tls(xr,yr,ne=ens_sizes[[0,1,3],mm],cn1=cn1,cn2=cn2,flag_3S=1)
        print (att_out[dataset]['betaCI'][0,:]*(mean_dec_warming[0,mm]-mean_dec_warming[1,mm]-mean_dec_warming[3,mm]))
        print (att_out[dataset]['betaCI'][1,:]*mean_dec_warming[1,mm])
        print (att_out[dataset]['betaCI'][2,:]*mean_dec_warming[3,mm])
        if mm == 0:
            label1='OTH'
            label2='NAT'
            label3='AER'
        ...
        plt.plot([mm+0.9,mm+0.9],numpy.transpose(att_out[dataset]['betaCI'][0,:]*(mean_dec_warming[0,mm]-mean_dec_warming[1,mm]-mean_dec_warming[3,mm])),color='green',linewidth=4,label=label1)
        plt.plot([mm+1,mm+1],numpy.transpose(att_out[dataset]['betaCI'][1,:]*mean_dec_warming[1,mm]),color='blue',linewidth=4,label=label2)
        plt.plot([mm+1.1,mm+1.1],numpy.transpose(att_out[dataset]['betaCI'][2,:]*mean_dec_warming[3,mm]),color='orange',linewidth=4,label=label3)
    ...
    plt.axis([0,nmodel+1,-1,2])
    plt.xticks([1,2],['CanESM5','CNRM-CM6-1'], size='small')
    plt.xlabel('Model')
    ...
    for mm, dataset in enumerate(grouped_input_data):
        if mm == 0:
            ...
            label2=None
            label3=None
        plt.plot([mm+0.9,mm+0.9],numpy.transpose(att_out[dataset]['betaCI'][0,:]),color='green',linewidth=4,label=label1)
        plt.plot([mm+1.0,mm+1.0],numpy.transpose(att_out[dataset]['betaCI'][1,:]),color='blue',linewidth=4,label=label2)
        plt.plot([mm+1.1,mm+1.1],numpy.transpose(att_out[dataset]['betaCI'][2,:]),color='orange',linewidth=4,label=label3)
    plt.plot([0,3],[0,0],color='black',linewidth=1,ls='--')
    ...
    for mm, dataset in enumerate(grouped_input_data):
        if mm == 0:
            ...
            myls=':'
        plt.plot(years,mean_diag[:,0,mm]-numpy.mean(mean_diag[0:5,0,mm]),color='red',linewidth=4,label=dataset+'_ALL',ls=myls)
        plt.plot(years,mean_diag[:,1,mm]-numpy.mean(mean_diag[0:5,1,mm]),color='blue',linewidth=4,label=dataset+'_NAT',ls=myls)
        plt.plot(years,mean_diag[:,2,mm]-numpy.mean(mean_diag[0:5,2,mm]),color='green',linewidth=4,label=dataset+'_GHG',ls=myls)
        plt.plot(years,mean_diag[:,3,mm]-numpy.mean(mean_diag[0:5,3,mm]),color='orange',linewidth=4,label=dataset+'_AER',ls=myls)
    plt.plot(years,had4_diag-numpy.mean(had4_diag[0:5]),color="Black",linewidth=4,label='HadCRUT4')
    plt.legend(loc="upper left")
    plt.xlabel('Year')

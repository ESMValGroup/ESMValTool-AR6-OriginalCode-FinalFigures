...
def select_latitudes(cubelist, start_lat=-90, end_lat=90):
    ...
    lat_constr = iris.Constraint(
        latitude=lambda cell: start_lat < cell <= end_lat)
...
def calculate_siextent(cubelist, threshold=15):
    ...
    for n, cube in enumerate(cubelist):
        ...
        # not beautiful but it works: here we create a new cube,
        # where data is a sum of area covered by
        # sea ice in the whole space that was provided.
        # The only coordinate is time, taken from the original
        ...
        new_cube = iris.cube.Cube(area, standard_name='sea_ice_extent',
...
def calculate_siarea(cubelist):
    ...
    for cube in cubelist:
        ...
        siconc = cube.data / 100  # since the data is in %,
        # it has to be converted into fraction
        area = np.ma.array(area, mask=cube.data.mask)

        sia_arr = siconc * area

        sia = (sia_arr.sum(axis=(1, 2)) / (
                1000 ** 2)) / 1000000
        # iris provides area in m, converting it to 10^6km2
        # for now passing parent cube attributes, clean before merging!!!
        sia_cube = iris.cube.Cube(sia, standard_name='sea_ice_area',
                                  long_name='sea ice area', var_name='siarea',
                                  units="1e6 km2", attributes=cube.attributes,
                                  dim_coords_and_dims=[(time, 0)])
...
def calculate_siparam(cubelist, siext=True):
    # function which determines if sea ice extent or sea ice are should be
    # calculated
...
def create_coords(cubelist, year_n):
    ...
    coord = [
        np.average(cb.coord('time').points[year_n * i:year_n * i + year_n]) for
        i in range(0, int(n_t / year_n))]
    bnds = [[cb.coord('time').bounds[year_n * i][0],
             cb.coord('time').bounds[year_n * i + (year_n - 1)][1]] for i in
    ...
    dcoord = iris.coords.DimCoord(np.asarray(coord), bounds=np.asarray(bnds),
                                  ...
                                  units=cb.coord('time').units,
                                  long_name=cb.coord('time').long_name,
...
def n_year_mean(cubelist, n):
    ...
    for cube in cubelist:
        ...
        if n_t % n != 0:
            ...
            logger.info(
                'The n of years is not divisible by %s last %s years '
                'were not taken into account',
                str(n), str(n_t % n))
        if len(cube.data.shape) == 1:
            data = [np.average(cube.data[n * i:n * i + n]) for i in
                    range(0, int(n_t / n))]
            n_aver_cube = iris.cube.Cube(np.asarray(data),
...
def substract_ref_period(cubelist, ref_period):
    constr = iris.Constraint(
        time=lambda cell: ref_period[0] <= cell.point.year <= ref_period[1])

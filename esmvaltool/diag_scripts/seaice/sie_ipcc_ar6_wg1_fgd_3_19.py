...
def ensemble_average(cubelist):
    ...
    if len(cubelist) == 1:
        ...
        averaged_cube = merged_cubelist.collapsed('realization', iris.analysis.MEAN)
...
def reform_data_dic(data_dic):
    ...
    for panel_id in data_dic.keys():
        ...
        for proj_name in data_dic[panel_id].keys():
            ...
            vb_month = []
            months_for_acs = list()
            for n, month in enumerate(data_dic[panel_id][proj_name].keys()):
                data_dic[panel_id][proj_name][month]['mme_mean'].add_aux_coord(
                    iris.coords.AuxCoord(n, long_name='idx', var_name='idx'))
                mean_merge_cubelist.append(data_dic[panel_id][proj_name][month]['mme_mean'])
                data_dic[panel_id][proj_name][month]['mme_significance'].add_aux_coord(
                    iris.coords.AuxCoord(n, long_name='idx', var_name='idx'))
                ...
                months_for_acs.append(month)
            mean_cube = mean_merge_cubelist.merge_cube()
            mean_cube.add_aux_coord(iris.coords.AuxCoord(months_for_acs,
                     long_name='month', var_name='month'), data_dims=0)
            sign_cube = sign_merge_cubelist.merge_cube()
            sign_cube.add_aux_coord(iris.coords.AuxCoord(months_for_acs,
                     long_name='month', var_name='month'), data_dims=0)
            reformed_dic[panel_id][proj_name] = {'means': mean_cube,
                                                 'significance': sign_cube,
                                                 'n_models': n_models,
...
def make_panel(data_dict, fig, inner, title='', panel_id='', put_ylabel=False):
    ...
    norm = mcolors.BoundaryNorm(np.arange(-1.5, 1.6, 0.3), tmp_cmap.N,
                                extend='both')
    ...
    for n_p, proj in enumerate(data_dict.keys()):
        ...
        pmesh = iplt.pcolormesh(data_dict[proj]['means'], axes=ax, norm=norm,
                                cmap=tmp_cmap)
        if panel_id != 'OBS':
            scat = ax.scatter((data_dict[proj]['significance'].coord('time').points *
               data_dict[proj]['significance'].data),
               data_dict[proj]['significance'].coord('idx').points[:, None] *
               data_dict[proj]['significance'].data, s=37, c='silver')
            if len(fig.legends)<1:
                fig.legend([scat], ['multi-model mean >\n'
                                    'inter-model spread'], loc=4, frameon=False, borderpad=1.6)
        ylims = ax.get_ylim()
        ax.set_ylim(ylims[::-1])
        ax.set_yticks(data_dict[proj]['means'].coord('idx').points[::-1])
        ...
        if put_ylabel:
            if data_dict[proj]['n_models'] >1:
                ax.set_ylabel(proj + ' [' + str(data_dict[proj]['n_models'])
                              + ']', rotation=0, labelpad=32)
            else:
                ax.set_ylabel(proj.split('-')[1], rotation=0, labelpad=32)
        else:
            ax.set_ylabel('')
        fig.add_subplot(ax)
    ...
    for ncol, hemisph in enumerate(data_dict.keys()):
        ...
        for nrow, panel_id in enumerate(panel_order):
            ...
            inner = gridspec.GridSpecFromSubplotSpec(nsubpan, 1,
                       subplot_spec=outer[nrow, ncol], wspace=0.0, hspace=0.05)
            if nrow == 0:
                if hemisph == 'NH':
                    title = 'Arctic SIA'
                else:
            ...
            ax, pmesh = make_panel(data_dict[hemisph][panel_id], fig, inner,
                                   title=title, panel_id=panel_id,
                                   put_ylabel=put_ylabel)

    outer.set_height_ratios(h_ratios)
    cax = fig.add_axes([0.36,0.06,0.4,0.01])
    cbar = fig.colorbar(pmesh, cax=cax, orientation='horizontal')
    cbar.ax.set_xlabel(r'10$^6$ km$^2$' +'\n SIA anomaly')
    fig.subplots_adjust(left=0.19, right=0.99, top=0.9, bottom=0.11, wspace=0.23)  # , hspace=0.2)
    fig.suptitle('Timeseries of observed and simulated sea ice area (SIA) anomalies',
                 fontsize='x-large', x=0.55)
    fig.text(0.17, 0.03, 'Reference period:\n'+ str(cfg['ref_period'][0]) +
             ' - ' + str(cfg['ref_period'][1]))

    for n_id, panel_id in enumerate(panel_order):
        bnds = outer[n_id,0].get_position(fig)
        crds = bnds.bounds
        x = crds[0]/15
        y = crds[1] + crds[3]/1.97
